...
class ScannerTest(unittest.TestCase):
    def setUp(self):
        ...
        scan_types = git.Repo.clone_from(
    ...
    def test_check_test_file(self):
        ...
        if len(missing_tests) > 0:
            print(
                "The following scanners are missing test cases or incorrectly named"
            )
    ...
    def test_check_for_doc(self):
        ...
        integration_index = (integration_text.index("Integrations") +
                             len("Integrations") + 1)
        usage_index = integration_text.index("Usage Examples") - len(
            "Models") - 2
        integration_text = integration_text[
            integration_index:usage_index].lower()
        integration_text = (integration_text.replace("_", " ").replace(
            "-", " ").replace(".", "").split("\n"))
        acronyms = []
    ...
    def test_check_for_fixtures(self):
        ...
        fixtures = [
            fix.replace("\t", "").replace("\n", "").replace(".", "").replace(
                "-", " ").strip().lower() for fix in fixtures
        ]
        remove_items = [
            "{",
            "},",
        ...
        for tool in self.tools:
            reg = re.compile(tool.replace("_", " "))
            matches = list(filter(reg.search, fixtures)) + list(
        ...
        if len(missing_fixtures) > 0:
            ...
            print(
                "https://github.com/DefectDojo/django-DefectDojo/blob/master/dojo/fixtures/test_type.json\n"
            )
    ...
    def test_check_for_forms(self):
        ...
        forms = [
            form.replace("(", "").replace(")", "").replace("-", " ").replace(
                '"', "").replace(".", "") for form in forms
        ]
    ...
    def test_check_for_options(self):
        ...
        templates = templates[templates.index("<ul>") +
                              1:templates.index("</ul>")]
        remove_patterns = ["<li><b>", "</b>", "</li>", " scanner", " scan"]
        for pattern in remove_patterns:
            templates = [re.sub(pattern, "", temp) for temp in templates]

        templates = [
            temp[:temp.index(" - ")] for temp in sorted(templates)
            if " - " in temp
        ]
        templates = [
    ...
    def test_engagement_import_scan_result(self):
        ...
        options_text = "".join(
            driver.find_element_by_name("scan_type").text).split("\n")
        ...
        mod_options = [
            scan.lower().replace("-", " ").replace(".", "")
            for scan in mod_options
        ]
        ...
        for test in self.tests:
            ...
            if len(found_matches) == 1:
                ...
                try:
                    ...
                    scan_map[test] = options_text[list(
                        found_matches.keys())[index]]
                except:
                    pass
        ...
        for test in self.tests:
            ...
            for case in cases:
                ...
                driver.find_element_by_name("_Import Scan Results").click()
                try:
                    driver.find_element_by_id("id_active").get_attribute(
                        "checked")
                    driver.find_element_by_id("id_verified").get_attribute(
                        "checked")
                    scan_type = scan_map[test]
                    Select(driver.find_element_by_id(
                        "id_scan_type")).select_by_visible_text(scan_type)
                    test_location = self.repo_path + "/" + test + "/" + case
                    driver.find_element_by_id("id_file").send_keys(
                        test_location)
                    driver.find_element_by_css_selector(
                        "input.btn.btn-primary").click()
                    EngagementTXT = "".join(
                        driver.find_element_by_tag_name("BODY").text).split(
                            "\n")
                    reg = re.compile("processed, a total of")
                    matches = list(filter(reg.search, EngagementTXT))
                    if len(matches) != 1:
                        failed_tests += [
                            test.upper() + " - " + case + ": Not imported"
                        ]
