...
def migrate_alerts(request):
    ...
    return render(request, "dojo/migrate_alerts.html", {"alerts": alerts})
...
def alerts_json(request, limit=None):
    ...
    if limit:
        alerts = serializers.serialize(
            "json",
            Alerts.objects.filter(user_id=request.user)[:int(limit)])
    else:
        alerts = serializers.serialize(
...
def add_user(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and contact_form.is_valid():
            ...
            contact.user = user
            contact.save()
            if ("authorized_products" in form.cleaned_data
...
def edit_user(request, uid):
    ...
    form = AddDojoUserForm(instance=user,
                           initial={"authorized_products": authed_products})
    if not request.user.is_superuser:
        form.fields["is_staff"].widget.attrs["disabled"] = True
        form.fields["is_superuser"].widget.attrs["disabled"] = True
        form.fields["is_active"].widget.attrs["disabled"] = True
