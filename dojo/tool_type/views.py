...
def new_tool_type(request):
    if request.method == "POST":
        ...
        tform = ToolTypeForm()
        add_breadcrumb(title="New Tool Type Configuration",
                       top_level=False,
                       request=request)
...
def edit_tool_type(request, ttid):
    ...
    add_breadcrumb(title="Edit Tool Type Configuration",
                   top_level=False,
                   request=request)

    return render(request, "dojo/edit_tool_type.html", {
        "tform": tform,
    })
...
def tool_type(request):
    ...
    add_breadcrumb(title="Tool Type List",
                   top_level=not len(request.GET),
                   request=request)
    return render(request, "dojo/tool_type.html", {
        "confs": confs,
    })
