...
def new_tool_config(request):
    if request.method == "POST":
        ...
        add_breadcrumb(title="New Tool Configuration",
                       top_level=False,
                       request=request)
    return render(request, "dojo/new_tool_config.html", {"tform": tform})
...
def tool_config(request):
    ...
    add_breadcrumb(title="Tool Configuration List",
                   top_level=not len(request.GET),
                   request=request)
    return render(request, "dojo/tool_config.html", {
        "confs": confs,
    })
