...
class EndPointViewSet(
    ...
    @detail_route(
        methods=["post"],
        permission_classes=[permissions.UserHasReportGeneratePermission],
    )
...
class FindingViewSet(
    ...
    def get_queryset(self):
        if not self.request.user.is_staff:
            return Finding.objects.filter(reporter_id__in=[self.request.user])
        else:
    ...
    def tags(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            if new_tags.is_valid():
                ...
                all_tags = serializers.TagSerializer({
                    "tags": all_tags
                }).data["tags"]
            ...
            for tag in del_tags:
                if tag not in all_tags:
                    return Response(
                        {
                            "error":
                            "'{}' is not a valid tag in list".format(tag)
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                all_tags.remove(tag)
...
class JiraConfigurationsViewSet(
        mixins.ListModelMixin,
        mixins.RetrieveModelMixin,
        mixins.DestroyModelMixin,
        mixins.UpdateModelMixin,
...
class ScanSettingsViewSet(
    ...
    permission_classes = (
        permissions.UserHasScanSettingsPermission,
        DjangoModelPermissions,
    )
    filter_backends = (DjangoFilterBackend, )
...
class ScansViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin,
                   viewsets.GenericViewSet):
    # TODO: ipscans
        ...
        for finding in findings.qs.order_by("numerical_severity"):
            ...
            if notes:
                finding_notes.append({
                    "finding_id": finding,
                    "notes":
                    notes.filter(private=False
                                 ),  # fetching only public notes for report
                })
