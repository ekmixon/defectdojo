...
class TagListSerializerField(serializers.ListField):
    ...
    default_error_messages = {
        ...
        "invalid_json":
        _("Invalid json list. A tag list submitted in string"
...
class TaggitSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        ...
        tag_object = super(TaggitSerializer,
...
class ProductSerializer(TaggitSerializer, serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        exclude = ("tid", "manager", "prod_manager", "tech_contact", "updated")
        extra_kwargs = {
            "authorized_users": {
                "queryset":
                User.objects.exclude(is_staff=True).exclude(is_active=False)
            }
        }
...
class TestCreateSerializer(TaggitSerializer, serializers.ModelSerializer):
    ...
    notes = serializers.PrimaryKeyRelatedField(allow_null=True,
                                               default=[],
...
class ImportScanSerializer(TaggitSerializer, serializers.Serializer):
    ...
    minimum_severity = serializers.ChoiceField(choices=SEVERITY_CHOICES,
                                               default="Info")
    ...
    engagement = serializers.PrimaryKeyRelatedField(
        queryset=Engagement.objects.all())
    lead = serializers.PrimaryKeyRelatedField(allow_null=True,
                                              default=None,
                                              queryset=User.objects.all())
    tags = TagListSerializerField(required=False)
    close_old_findings = serializers.BooleanField(required=False,
                                                  default=False)
    ...
    def save(self):
        ...
        if close_old_findings:
            ...
            if test.engagement.deduplication_on_engagement:
                old_findings = (Finding.objects.exclude(test=test).exclude(
                    hash_code__in=new_hash_codes).exclude(
                        hash_code__in=skipped_hashcodes).filter(
            ...
            for old_finding in old_findings:
                ...
                old_finding.save()
                title = 'An old finding has been closed for "{}".'.format(
                    test.engagement.product.name)
                description = 'See <a href="{}">{}</a>'.format(
                    reverse("view_finding", args=(old_finding.id, )),
                    old_finding.title)
                create_notification(
                    event="other",
                    title=title,
            ...
            for finding in to_mitigate:
                ...
                if settings.USE_TZ:
                    finding.mitigated = timezone.make_aware(
                        finding.mitigated, timezone.get_default_timezone())
...
class NoteSerializer(serializers.ModelSerializer):
    ...
    editor = UserSerializer(read_only=True, many=False)
...
class ExecutiveSummarySerializer(serializers.Serializer):
    ...
    test_strategy_ref = serializers.URLField(max_length=200,
                                             min_length=None,
                                             allow_blank=True)
...
class ReportGenerateSerializer(serializers.Serializer):
    ...
    finding_images = FindingToFindingImagesSerializer(many=True,
                                                      allow_null=True)
