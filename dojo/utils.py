...
def sync_false_history(new_finding, *args, **kwargs):
    if new_finding.endpoints.count() == 0:
        ...
        ).exclude(id=new_finding.id).exclude(cwe=None))
        ...
        eng_findings_cwe = (Finding.objects.filter(
            ...
            test__test_type=new_finding.test.test_type,
            false_p=True,
        ).exclude(id=new_finding.id).exclude(cwe=None).exclude(endpoints=None))
        eng_findings_title = (Finding.objects.filter(
            test__engagement__product=new_finding.test.engagement.product,
            title=new_finding.title,
            test__test_type=new_finding.test.test_type,
            false_p=True,
        ).exclude(id=new_finding.id).exclude(endpoints=None))
    total_findings = eng_findings_cwe | eng_findings_title
...
def is_deduplication_on_engagement_mismatch(new_finding, to_duplicate_finding):
    return (not new_finding.test.engagement.deduplication_on_engagement and
    ...
    if system_settings.enable_deduplication:
        ...
        if new_finding.test.engagement.deduplication_on_engagement:
            eng_findings_cwe = (Finding.objects.filter(
                ...
                cwe=new_finding.cwe).exclude(id=new_finding.id).exclude(
                    cwe=0).exclude(duplicate=True))
            eng_findings_title = (Finding.objects.filter(
                test__engagement=new_finding.test.engagement,
                title=new_finding.title,
            ).exclude(id=new_finding.id).exclude(duplicate=True))
        else:
            eng_findings_cwe = (Finding.objects.filter(
                test__engagement__product=new_finding.test.engagement.product,
                cwe=new_finding.cwe,
            ).exclude(id=new_finding.id).exclude(cwe=0).exclude(
                duplicate=True))
            eng_findings_title = (Finding.objects.filter(
                test__engagement__product=new_finding.test.engagement.product,
                title=new_finding.title,
        ...
        deduplicationLogger.debug(
            "Found " + str(len(eng_findings_cwe)) +
            " findings with same cwe, " + str(len(eng_findings_title)) +
            " findings with same title: " + str(len(total_findings)) +
        ...
        for find in total_findings:
            ...
            # ---------------------------------------------------------
            if find.endpoints.count() != 0 and new_finding.endpoints.count(
            ) != 0:
                ...
                if (str(find.line) == str(new_finding.line)
                        and find.file_path == new_finding.file_path):
                    flag_line_path = True
                else:
                    deduplicationLogger.debug(
                        "no endpoints on one of the findings and file_path doesn't match"
                    )
            else:
                deduplicationLogger.debug(
            ...
            if find.hash_code == new_finding.hash_code:
                flag_hash = True
            deduplicationLogger.debug("deduplication flags for new finding " +
                                      str(new_finding.id) +
                                      " and existing finding " + str(find.id) +
                                      " flag_endpoints: " +
                                      str(flag_endpoints) +
                                      " flag_line_path:" +
                                      str(flag_line_path) + " flag_hash:" +
                                      str(flag_hash))
            # ---------------------------------------------------------
            ...
            if (flag_endpoints or flag_line_path) and flag_hash:
                deduplicationLogger.debug(
                    "New finding " + str(new_finding.id) +
                    " is a duplicate of existing finding " + str(find.id))
                ...
                if getattr(new_finding, rule.match_field) == rule.match_text:
                    if rule.application == "Append":
                        set_attribute_rule(
                            new_finding,
                            rule,
                            (getattr(new_finding, rule.applied_field) +
                             rule.text),
                        )
                    else:
    ...
    for i in range(6):
        ...
        for f in findings:
            if (f.mitigated is not None
                    and end_of_period >= f.mitigated >= start_of_period):
                o_count["closed"] += 1
            elif (f.mitigated is not None and f.mitigated > end_of_period
...
def add_breadcrumb(parent=None,
    ...
    if top_level or crumbs is None:
        ...
        for crumb in crumbs:
            crumb_to_resolve = (crumb["url"] if "?" not in crumb["url"] else
                                crumb["url"][:crumb["url"].index("?")])
            ...
            for obj_crumb in obj_crumbs:
                obj_crumb_to_resolve = (
                    ...
                    obj_crumb["url"][:obj_crumb["url"].index("?")])
        ...
        for finding in findings:
            try:
                if (new_date < datetime.combine(
                    ...
                    if days[day_offset[
                            finding.date.weekday()]] > highest_count:
...
def opened_in_period(start_date, end_date, pt):
    ...
    ).values("numerical_severity").annotate(
...
def get_jira_connection(finding):
    jira = None
    prod = Product.objects.get(engagement=Engagement.objects.get(
        test=finding.test))
...
def add_issue(find, push_to_jira):
    ...
    if push_to_jira:
        if "Active" in find.status() and "Verified" in find.status():
            ...
            log_jira_alert("Finding not active or not verified.", find)
    ...
    if jira_check_attachment(issue, basename) is False:
        try:
            if jira_filename is not None:
                ...
                jira.add_attachment(issue=issue,
...
def update_issue(find, old_status, push_to_jira):
    ...
    if push_to_jira:
        ...
        if ("Inactive" in find.status() or "Mitigated" in find.status()
                ...
                or "Out of Scope" in find.status()
                or "Duplicate" in find.status()):
    ...
    if jpkey.enable_engagement_epic_mapping and push_to_jira:
        ...
        try:
            ...
            if "customfield" in error:
                message = (
                    "The 'Epic name id' in your DefectDojo Jira Configuration does not appear to be correct. Please visit, "
                    + jira_conf.url +
...
def dojo_crypto_encrypt(plaintext):
    ...
    if plaintext:
        ...
        iv = os.urandom(16)
        data = prepare_for_save(iv, encrypt(key, iv,
...
def prepare_for_save(iv, encrypted_value):
    ...
    if encrypted_value and encrypted_value is not None:
        ...
        stored_value = ("AES.1:" + binascii.b2a_hex(iv).decode("utf-8") + ":" +
    ...
    def create_description(event):
        if "description" not in kwargs.keys():
            if event == "product_added":
                kwargs["description"] = ("Product " + kwargs["title"] +
    ...
    def send_mail_notification(address):
        ...
        try:
            email = EmailMessage(
                ...
                headers={
                    "From":
                    "{}".format(get_system_setting("mail_notifications_from"))
                },
            )
...
def calculate_grade(product):
    ...
    if system_settings.enable_product_grade:
        ...
        ).values("severity").annotate(Count("numerical_severity")).order_by())
...
class Product_Tab:
    def __init__(self, product_id, title=None, tab=None):
        ...
        self.title = title
        self.tab = tab
        self.engagement_count = Engagement.objects.filter(product=self.product,
                                                          active=True).count()
...
def apply_cwe_to_template(finding, override=False):
    if System_Settings.objects.get().enable_template_match or override:
        ...
        template = Finding_Template.objects.filter(
            cwe=finding.cwe,
            title__icontains=finding.title,
            template_match=True).first()
