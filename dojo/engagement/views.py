...
def engagement_calendar(request):
    ...
    add_breadcrumb(title="Engagement Calendar",
                   top_level=True,
                   request=request)
    return render(
        request,
...
def engagement(request):
    ...
    add_breadcrumb(title="Active Engagements",
                   top_level=not len(request.GET),
                   request=request)

    return render(
        request,
        "dojo/engagement.html",
        {
            "products": prods,
            "filtered": filtered,
...
def edit_engagement(request, eid):
    ...
    if request.method == "POST":
        ...
        if (get_system_setting("enable_jira") and
                JIRA_PKey.objects.filter(product=eng.product).count() != 0):
...
def view_engagement(request, eid):
    ...
    eng_findings = (Finding.objects.filter(
        test__in=eng.test_set.all()).exclude(
            id__in=exclude_findings).order_by("title"))
            ...
            try:
                for item in parser.items:
                    ...
                    if (hasattr(item, "unsaved_req_resp")
                        ...
                        for req_resp in item.unsaved_req_resp:
                            if form.get_scan_type() == "Arachni Scan":
                                ...
                                burp_rr = BurpRawRequestResponse(
                                    finding=item,
                                    burpRequestBase64=req_resp["req"].encode(
                                        "utf-8"),
                                    burpResponseBase64=req_resp["resp"].encode(
                                        "utf-8"),
                                )
                            burp_rr.clean()
                            burp_rr.save()
                    ...
                    if (item.unsaved_request is not None
                        ...
                        burp_rr = BurpRawRequestResponse(
                            finding=item,
                            burpRequestBase64=item.unsaved_request.encode(
                                "utf-8"),
                            burpResponseBase64=item.unsaved_response.encode(
                                "utf-8"),
                        )
                        burp_rr.clean()
                        burp_rr.save()
                ...
                create_notification(
                    event="results_added",
                    title=str(finding_count) + " findings for " +
...
def view_risk(request, eid, raid):
    ...
    findings = (Finding.objects.filter(test__in=eng.test_set.all()).exclude(
        id__in=exclude_findings).order_by("title"))
