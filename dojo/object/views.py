...
def view_objects(request, pid):
    object_queryset = Objects.objects.filter(product=pid).order_by(
        "path", "folder", "artifact")

    product_tab = Product_Tab(
        pid,
        title="Tracked Product Files, Paths and Artifacts",
        tab="settings")
    return render(
        request,
...
def view_object_eng(request, id):
    object_queryset = Objects_Engagement.objects.filter(
        engagement=id).order_by("object_id__path", "object_id__folder",
                                "object_id__artifact")
    engagement = Engagement.objects.get(id=id)
    product_tab = Product_Tab(
        engagement.product.id,
        title="Tracked Files, Folders and Artifacts on a Product",
        tab="engagements",
    )
