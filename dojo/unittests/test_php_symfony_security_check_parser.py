...
class TestPhpSymfonySecurityCheckerParser(TestCase):
    def test_php_symfony_security_check_parser_without_file_has_no_findings(
            self):
        parser = PhpSymfonySecurityCheckParser(None, Test())
        self.assertEqual(0, len(parser.items))

    def test_php_symfony_security_check_parser_with_no_vuln_has_no_findings(
            self):
        testfile = open(
    ...
    def test_php_symfony_security_check_parser_with_many_vuln_has_many_findings(
            self):
        testfile = open(
