...
class TestAddEndpointMetaData(TestCase):
    ...
    def make_request(self, user_is_staff, id, data=None):
        ...
        if data:
            request = EndpointMetaDataTestUtil.create_post_request(
                user, self.add_meta_data_url, data)
        else:
            request = EndpointMetaDataTestUtil.create_get_request(
                user, self.add_meta_data_url)
...
class TestEditEndpointMetaData(TestCase):
    ...
    def make_request(self, user_is_staff, id, data=None):
        ...
        if data:
            request = EndpointMetaDataTestUtil.create_post_request(
                user, self.edit_meta_data_url, data)
        else:
            request = EndpointMetaDataTestUtil.create_get_request(
                user, self.edit_meta_data_url)
