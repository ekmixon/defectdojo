        ...
        if "issue" in list(parsed.keys()):
            ...
            if jissue.finding is not None:
                ...
                if finding.active == resolved:
                    if finding.active:
                        if (jira_conf and resolution["name"]
                                in jira_conf.accepted_resolutions):
                        ...
                        elif (jira_conf and resolution["name"]
                              in jira_conf.false_positive_resolutions):
                            finding.active = False
    ...
    if request.method == "POST":
        ...
        if jform.is_valid():
            try:
                ...
                key_url = (jira_server + "/rest/api/latest/issue/" + issue_id +
                ...
                create_notification(
                    event="other",
                    title="New addition of JIRA URL %s" %
...
def jira(request):
    ...
    add_breadcrumb(title="JIRA List",
                   top_level=not len(request.GET),
                   request=request)
    return render(request, "dojo/jira.html", {
        "confs": confs,
    })
