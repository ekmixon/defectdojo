...
class SelectWithPop(forms.Select):
    def render(self, name, *args, **kwargs):
        ...
        popup_plus = (
            '<div class="input-group dojo-input-group">' + html +
            '<span class="input-group-btn"><a href="/' + name +
...
class MultipleSelectWithPopPlusMinus(forms.SelectMultiple):
    def render(self, name, *args, **kwargs):
        html = super(MultipleSelectWithPopPlusMinus,
                     self).render(name, *args, **kwargs)
        popup_plus = (
            '<div class="input-group dojo-input-group">' + html +
            '<span class="input-group-btn"><a href="/' + name +
            '/add" class="btn btn-primary" class="add-another" id="add_id_' +
            name +
            '" onclick="return showAddAnotherPopup(this);"><span class="icon-plusminus"></span></a></span></div>'
        )
...
class ProductForm(forms.ModelForm):
    ...
    authorized_users = forms.ModelMultipleChoiceField(queryset=None,
                                                      required=False,
                                                      label="Authorized Users")

    def __init__(self, *args, **kwargs):
        non_staff = User.objects.exclude(is_staff=True).exclude(
            is_active=False)
...
class ImportScanForm(forms.Form):
    ...
    active = forms.BooleanField(
        help_text="Select if these findings are currently active.",
        required=False)
    verified = forms.BooleanField(
        help_text="Select if these findings have been verified.",
        required=False)
    scan_type = forms.ChoiceField(required=True,
                                  choices=SORTED_SCAN_TYPE_CHOICES)
    tags = forms.CharField(
        widget=forms.SelectMultiple(choices=[]),
        required=False,
...
class MergeFindings(forms.ModelForm):
    ...
    append_description = forms.BooleanField(
        ...
        required=False,
        help_text=
        "Description in all findings will be appended into the merged finding.",
    )
    ...
    add_endpoints = forms.BooleanField(
        ...
        required=False,
        help_text=
        "Endpoints in all findings will be merged into the merged finding.",
    )
    ...
    dynamic_raw = forms.BooleanField(
        ...
        required=False,
        help_text=
        "Dynamic scanner raw requests in all findings will be merged into the merged finding.",
    )
    ...
    tag_finding = forms.BooleanField(
        ...
        required=False,
        help_text=
        "Tags in all findings will be merged into the merged finding.",
    )
    ...
    mark_tag_finding = forms.BooleanField(
        ...
        required=False,
        help_text=
        "Creates a tag titled 'merged' for the finding that will be merged. If the 'Finding Action' is set to 'inactive' the inactive findings will be tagged with 'merged-inactive'.",
    )
    ...
    append_reference = forms.BooleanField(
        ...
        required=False,
        help_text=
        "Reference in all findings will be appended into the merged finding.",
    )
    ...
    finding_action = forms.ChoiceField(
        ...
        help_text=
        "The action to take on the merged finding. Set the findings to inactive or delete the findings.",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["finding_to_merge_into"] = forms.ModelChoiceField(
            ...
            help_text=
            "Findings selected below will be merged into this finding.",
        )
...
class UploadRiskForm(forms.ModelForm):
    ...
    accepted_findings = forms.ModelMultipleChoiceField(
        ...
        widget=forms.widgets.SelectMultiple(attrs={"size": 10}),
        help_text=(
            "Active, verified findings listed, please select to add findings."
        ),
    )
    reporter = forms.ModelChoiceField(queryset=User.objects.exclude(
        username="root"))
    accepted_by = forms.CharField(
        help_text="The entity or person that accepts the risk.",
        required=False)
    expiration_date = forms.DateTimeField(
        label="Date Risk Exception Expires",
        required=False,
        widget=forms.TextInput(attrs={"class": "datepicker"}),
    )
    compensating_control = forms.CharField(
        label="Compensating Control",
        help_text=
        "Compensating control (if applicable) for this risk exception",
        required=False,
        max_length=2400,
...
class ScanSettingsForm(forms.ModelForm):
    ...
    options = (("Weekly", "Weekly"), ("Monthly", "Monthly"), ("Quarterly",
                                                              "Quarterly"))
    frequency = forms.ChoiceField(choices=options)
    prots = [("TCP", "TCP"), ("UDP", "UDP")]
    protocol = forms.ChoiceField(choices=prots, help_text=proHelpTxt)
...
class DeleteIPScanForm(forms.ModelForm):
    ...
    class Meta:
        model = Scan
        exclude = ("scan_settings", "date", "protocol", "status", "baseline")


class VaForm(forms.ModelForm):
    addresses = forms.CharField(max_length=2000, widget=forms.Textarea)
    options = (("Immediately", "Immediately"), ("6AM", "6AM"), ("10PM",
                                                                "10PM"))
...
class EngForm(forms.ModelForm):
    name = forms.CharField(
        ...
        "Without a name the target start date will be used in " + "listings.",
    )
    description = forms.CharField(
        widget=forms.Textarea(attrs={}),
        required=False,
        help_text=
        "Description of the engagement and details regarding the engagement.",
    )
    tags = forms.CharField(
        widget=forms.SelectMultiple(choices=[]),
        required=False,
    ...
    target_end = forms.DateField(widget=forms.TextInput(attrs={
        "class": "datepicker",
        "autocomplete": "off"
    }))
...
class AddFindingForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (cleaned_data["active"]
                or cleaned_data["verified"]) and cleaned_data["duplicate"]:
...
class FindingTemplateForm(forms.ModelForm):
    apply_to_findings = forms.BooleanField(
        required=False,
        help_text=
        "Apply template to all findings that match this CWE. (Update will overwrite mitigation, impact and references for any active, verified findings.)",
    )
    title = forms.CharField(max_length=1000, required=True)
    tags = forms.CharField(
        widget=forms.SelectMultiple(choices=[]),
...
class AddEndpointForm(forms.Form):
    endpoint = forms.CharField(
        ...
        label="Endpoint(s)",
        help_text=
        "The IP address, host name or full URL. You may enter one endpoint per line. "
        "Each must be valid.",
    ...
    product = forms.CharField(
        ...
        help_text="The product this endpoint should be "
        "associated with.",
    )
    tags = forms.CharField(
        widget=forms.SelectMultiple(choices=[]),
                ...
                try:
                    ...
                    try:
                        ...
                        if "/" in host or "?" in host or "#" in host:
                            ...
                            p, host, path, query, fragment = urlsplit(
                                host_with_protocol)
                        self.endpoints_to_process.append(
                            [protocol, host, path, query, fragment])
                    except forms.ValidationError:
...
class CloseFindingForm(forms.ModelForm):
    entry = forms.CharField(
        ...
        label="Notes:",
        error_messages={
            "required": ("The reason for closing a finding is "
...
class DefectFindingForm(forms.ModelForm):
    CLOSE_CHOICES = (("Close Finding", "Close Finding"), ("Not Fixed",
                                                          "Not Fixed"))
...
class ClearFindingReviewForm(forms.ModelForm):
    entry = forms.CharField(
        ...
        label="Notes:",
        error_messages={
            "required": ("The reason for clearing a review is "
                         "required, please use the text area "
                         "below to provide documentation.")
        },
    )
...
class ReviewFindingForm(forms.Form):
    reviewers = forms.ModelMultipleChoiceField(
        queryset=Dojo_User.objects.filter(is_staff=True, is_active=True),
    ...
    entry = forms.CharField(
        ...
        label="Notes:",
        error_messages={
            "required": ("The reason for requesting a review is "
                         "required, please use the text area "
                         "below to provide documentation.")
        },
    )
...
class ProductTypeCountsForm(forms.Form):
    month = forms.ChoiceField(choices=list(MONTHS.items()),
                              required=True,
...
class ReportOptionsForm(forms.Form):
    yes_no = (("0", "No"), ("1", "Yes"))
    include_finding_notes = forms.ChoiceField(choices=yes_no,
                                              label="Finding Notes")
    include_finding_images = forms.ChoiceField(choices=yes_no,
                                               label="Finding Images")
    include_executive_summary = forms.ChoiceField(choices=yes_no,
                                                  label="Executive Summary")
    include_table_of_contents = forms.ChoiceField(choices=yes_no,
                                                  label="Table of Contents")
    report_type = forms.ChoiceField(choices=(("HTML", "HTML"), ("AsciiDoc",
                                                                "AsciiDoc")))
...
FindingImageFormSet = modelformset_factory(FindingImage,
                                           extra=3,
                                           max_num=10,
                                           exclude=[""],
                                           can_delete=True)
...
class ExpressJIRAForm(forms.ModelForm):
    ...
    issue_key = forms.CharField(
        required=True,
        help_text=
        "A valid issue ID is required to gather the necessary information.",
    )
...
class ToolConfigForm(forms.ModelForm):
    tool_type = forms.ModelChoiceField(queryset=Tool_Type.objects.all(),
                                       label="Tool Type")
    ssh = forms.CharField(widget=forms.Textarea(attrs={}),
                          required=False,
                          label="SSH Key")
...
class ToolProductSettingsForm(forms.ModelForm):
    ...
    class Meta:
        model = Tool_Product_Settings
        fields = [
            "name", "description", "url", "tool_configuration",
            "tool_project_id"
        ]
        exclude = ["tool_type"]
        order = ["name"]
...
RuleFormSet = modelformset_factory(Child_Rule,
                                   extra=2,
                                   max_num=10,
                                   exclude=[""],
                                   can_delete=True)
...
class JIRAPKeyForm(forms.ModelForm):
    conf = forms.ModelChoiceField(queryset=JIRA_Conf.objects.all(),
