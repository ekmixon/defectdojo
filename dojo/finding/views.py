...
from dojo.filters import (
    OpenFindingFilter,
    OpenFingingSuperFilter,
    AcceptedFingingSuperFilter,
...
from dojo.utils import (
    ...
    add_comment,
    jira_get_resolution_id,
    jira_change_resolution_id,
    get_jira_connection,
    get_system_setting,
    create_notification,
    ...
    if pid:
        if view == "All":
            ...
            findings = Finding.objects.filter(
                test__engagement__product__id=pid,
                active=True,
        ...
        if view == "All":
            filter_name = "All"
            findings = Finding.objects.filter(
                test__engagement=eid).order_by("numerical_severity")
        else:
            findings = Finding.objects.filter(
                test__engagement=eid, active=True,
                duplicate=False).order_by("numerical_severity")
    else:
        if view == "All":
            filter_name = "All"
            ...
            findings = Finding.objects.filter(
                active=True, duplicate=False).order_by("numerical_severity")

    if request.user.is_staff:
        findings = OpenFingingSuperFilter(request.GET,
                                          queryset=findings,
                                          user=request.user,
                                          pid=pid)
    else:
        ...
        findings = OpenFindingFilter(request.GET,
                                     queryset=findings,
                                     user=request.user,
                                     pid=pid)
...
def view_finding(request, fid):
    ...
    findings = Finding.objects.filter(
        test=finding.test).order_by("numerical_severity")
    try:
    ...
    product_tab = Product_Tab(finding.test.engagement.product.id,
                              title="View Finding",
                              tab="findings")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if source == "test":
                ...
                return HttpResponseRedirect(
                    reverse(
                        "product_open_findings",
                        args=(new_finding.test.engagement.product.id, ),
                    ) + "?test__engagement__product=" +
                    str(new_finding.test.engagement.product.id) + page_value)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            create_notification(
                ...
                description=
                'User %s has requested that users %s review the finding "%s" for accuracy:\n\n%s'
...
def find_template_to_apply(request, fid):
    ...
    templates_by_CVE = (Finding_Template.objects.annotate(
        cve_len=Length("cve"),
        order=models.Value(1, models.IntegerField())).filter(
            cve=finding.cve, cve_len__gt=0).order_by("-last_used"))
    if templates_by_CVE.count() == 0:

        templates_by_last_used = (
            Finding_Template.objects.all().order_by("-last_used").annotate(
                cve_len=Length("cve"),
        ...
        templates_by_last_used = (Finding_Template.objects.all().exclude(
            cve=finding.cve).order_by("-last_used").annotate(
                cve_len=Length("cve"),
                order=models.Value(2, models.IntegerField())))
...
def promote_to_finding(request, fid):
    ...
    if (get_system_setting("enable_jira") and
            JIRA_PKey.objects.filter(product=test.engagement.product) != 0):
...
def export_templates_to_json(request):
    leads_as_json = serializers.serialize("json",
        ...
        if template.template_match_title:
            ...
            if result_list:
                count = Finding.objects.filter(
                    ...
                    cwe=template.cwe).exclude(id__in=result_list)
            else:
                count = Finding.objects.filter(active=True,
                                               verified=True,
                                               cwe=template.cwe)
...
def manage_images(request, fid):
    ...
    if request.method == "POST":
        images_formset = FindingImageFormSet(request.POST,
                                             request.FILES,
                                             queryset=finding.images.all())
        if images_formset.is_valid():
            ...
            files = os.listdir(
                os.path.join(settings.MEDIA_ROOT, "finding_images"))

            for file in files:
                with_media_root = os.path.join(settings.MEDIA_ROOT,
                                               "finding_images", file)
    ...
    product_tab = Product_Tab(
        finding.test.engagement.product.id,
        title="Manage Finding Images",
        tab="findings",
    )
    return render(
        request,
        "dojo/manage_images.html",
        {
            "product_tab": product_tab,
...
def merge_finding_product(request, pid):
    ...
    if (request.GET.get("merge_findings")
            or request.method == "POST") and finding_to_update:
        finding = Finding.objects.get(id=finding_to_update[0],
                                      test__engagement__product=product)
        findings = Finding.objects.filter(id__in=finding_to_update,
                                          test__engagement__product=product)
        form = MergeFindings(
            finding=finding,
            findings=findings,
        ...
        if request.method == "POST":
            form = MergeFindings(request.POST,
                                 finding=finding,
                                 findings=findings)
            if form.is_valid():
                finding_to_merge_into = form.cleaned_data[
                ...
                if finding_to_merge_into not in findings_to_merge:
                    for finding in findings_to_merge.exclude(
                        ...
                        if form.cleaned_data["dynamic_raw"]:
                            BurpRawRequestResponse.objects.filter(
                                finding=finding).update(
                                    finding=finding_to_merge_into)
                        ...
                        if form.cleaned_data["finding_action"] == "inactive":
                            ...
                            note = Notes(entry=single_finding_notes_entry,
                                         author=request.user)
                            note.save()
                    ...
                    if form.cleaned_data["finding_action"] == "inactive":
                        ...
                        findings_to_merge.exclude(
                            pk=finding_to_merge_into.pk).update(
                                active=False,
                    ...
                    return HttpResponseRedirect(
                        reverse("edit_finding",
                                args=(finding_to_merge_into.id, )))
                else:
                    messages.add_message(
                        request,
                        messages.ERROR,
    ...
    custom_breadcrumb = {
        ...
        "?test__engagement__product=" + str(finding.test.engagement.product.id)
    }
        ...
        if request.POST.get("delete_bulk_findings") and finding_to_update:
            ...
            product_calc = list(
                Product.objects.filter(
                    engagement__test__finding__id__in=finding_to_update).
                distinct())
            finds.delete()
            ...
            if form.is_valid() and finding_to_update:
                finding_to_update = request.POST.getlist("finding_to_update")
                finds = Finding.objects.filter(
                    id__in=finding_to_update).order_by(
                ...
                for finding in finds:
                    if (JIRA_PKey.objects.filter(
                            product=finding.test.engagement.product).count() ==
                            0):
                        log_jira_alert(
