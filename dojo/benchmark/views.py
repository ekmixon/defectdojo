...
def return_score(queryset):
    ...
    for item in queryset:
        ...
        asvs_level_1_benchmark = asvs_level_1_benchmark + item[
            "pass_fail__count"]
...
def score_asvs(product, benchmark_type):
    ...
    asvs_level_1 = (Benchmark_Product.objects.filter(
        enabled=True,
    ...
    asvs_level_3 = (Benchmark_Product.objects.filter(
        ~Q(control__level_1=True),
...
def benchmark_view(request, pid, type, cat=None):
    ...
    benchmark_category = Benchmark_Category.objects.filter(
        type=type, enabled=True).order_by("name")
    category_name = ""

    # Add requirements to the product
    add_benchmark(
        Benchmark_Requirement.objects.filter(category__type=type,
                                             category__type__enabled=True,
                                             enabled=True).all(),
        product,
    )

    if cat:
        category_name = Benchmark_Category.objects.get(id=cat,
                                                       enabled=True).name
    ...
    new_benchmarks = Benchmark_Requirement.objects.filter(
        category__type=type, category__type__enabled=True,
        enabled=True).exclude(id__in=Benchmark_Product.objects.filter(
            product=product).values_list("control_id", flat=True))
    add_benchmark(new_benchmarks, product)

    Benchmark_ProductFormSet = modelformset_factory(
        Benchmark_Product, exclude=["product, control"], extra=0)

    if request.method == "POST":
        form = Benchmark_ProductFormSet(request.POST)
        summary_form = Benchmark_Product_SummaryForm(
    ...
    if cat:
        benchmarks = (Benchmark_Product.objects.filter(
            product=product.id,
            control__category=cat,
            control__category__enabled=True,
            control__category__type=type,
            control__enabled=True,
        ).all().order_by("control__objective_number"))

        benchmark_formset = Benchmark_ProductFormSet(
            queryset=Benchmark_Product.objects.filter(
                product=product.id,
                control__category=cat,
                control__category__enabled=True,
                control__category__type=type,
        ...
        benchmarks = (Benchmark_Product.objects.filter(
            ...
            control__category__type=type,
            control__enabled=True,
        ).all().order_by("control__category__name",
                         "control__objective_number"))
