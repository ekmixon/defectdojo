...
def new_cred(request):
    if request.method == "POST":
        ...
        tform = CredUserForm()
        add_breadcrumb(title="New Credential",
                       top_level=False,
                       request=request)
...
def view_cred_product(request, pid, ttid):
    ...
    add_breadcrumb(title="Credential Manager",
                   top_level=False,
                   request=request)
    cred_type = "Product"
    view_link = reverse("view_cred_product", args=(
        cred.product.id,
        cred.id,
    ))
    edit_link = reverse("edit_cred_product", args=(
        cred.product.id,
        cred.id,
    ))
    delete_link = reverse("delete_cred_product",
                          args=(
                              cred.product.id,
                              cred.id,
                          ))
...
def view_cred_product_engagement(request, eid, ttid):
    ...
    edit_link = ""
    view_link = reverse("view_cred_product_engagement", args=(
        eid,
        cred.id,
    ))
    delete_link = reverse("delete_cred_engagement", args=(
        eid,
        cred.id,
    ))
...
def view_cred_engagement_test(request, tid, ttid):
    ...
    cred_type = "Test"
    edit_link = None
    view_link = reverse("view_cred_engagement_test", args=(
        tid,
        cred.id,
    ))
    delete_link = reverse("delete_cred_test", args=(
        tid,
        cred.id,
    ))
...
def view_cred_finding(request, fid, ttid):
    ...
    cred_type = "Finding"
    edit_link = None
    view_link = reverse("view_cred_finding", args=(
        fid,
        cred.id,
    ))
    delete_link = reverse("delete_cred_finding", args=(
        fid,
        cred.id,
    ))
...
def new_cred_product_engagement(request, eid):
    ...
    add_breadcrumb(title="Add Credential Configuration",
                   top_level=False,
                   request=request)
...
def view_selenium(request, ttid):
    ...
    response["Content-Disposition"] = ("attachment; filename=selenium_script" +
                                       fileExtension)
