...
class Regulation(models.Model):
    ...
    description = models.TextField(
        blank=True, help_text=_("Information about the regulation's purpose."))
    reference = models.URLField(
        blank=True, help_text=_("An external URL for more information."))
...
class System_Settings(models.Model):
    ...
    enable_jira = models.BooleanField(default=False,
                                      ...
                                      blank=False)
    enable_jira_web_hook = models.BooleanField(
        default=False,
        verbose_name=
        "Enable JIRA web hook. Please note: It is strongly recommended to whitelist the Jira server using a proxy such as Nginx.",
        blank=False,
    )
    jira_choices = (
        ("Critical", "Critical"),
    ...
    jira_minimum_severity = models.CharField(max_length=20,
                                             blank=True,
                                             null=True,
                                             choices=jira_choices,
                                             default="None")
    jira_labels = models.CharField(
        max_length=200,
        blank=True,
        null=True,
        help_text="JIRA issue labels space seperated",
    )
    enable_slack_notifications = models.BooleanField(
        default=False, verbose_name="Enable Slack notifications", blank=False)
    slack_channel = models.CharField(max_length=100, default="", blank=True)
    slack_token = models.CharField(
        max_length=100,
        default="",
        blank=True,
    ...
    enable_hipchat_notifications = models.BooleanField(
        default=False,
        verbose_name="Enable HipChat notifications",
        blank=False)
    hipchat_site = models.CharField(
        max_length=100,
        default="",
        blank=True,
    ...
    hipchat_token = models.CharField(
        max_length=100,
        default="",
        blank=True,
    ...
    mail_notifications_from = models.CharField(max_length=200,
                                               ...
                                               blank=True)
    mail_notifications_to = models.CharField(max_length=200,
                                             default="",
                                             blank=True)
    s_finding_severity_naming = models.BooleanField(
        default=False,
        blank=False,
    ...
    false_positive_history = models.BooleanField(
        default=False,
        help_text=
        "DefectDojo will automatically mark the finding as a false positive if the finding has been previously marked as a false positive.",
    )
    ...
    display_endpoint_uri = models.BooleanField(
        default=False,
        verbose_name="Display Endpoint Full URI",
        help_text="Displays the full endpoint URI in the endpoint view.",
    )
    enable_product_grade = models.BooleanField(
        default=False,
        verbose_name="Enable Product Grading",
        help_text=
    ...
    product_grade_a = models.IntegerField(
        default=90,
    ...
    product_grade_d = models.IntegerField(default=60,
                                          ...
                                          help_text="Percentage score for a "
                                          "'D' >=")
    product_grade_f = models.IntegerField(
        default=59,
...
class UserContactInfo(models.Model):
    ...
    slack_username = models.CharField(
        ...
        null=True,
        max_length=150,
        help_text="Email address associated with your slack account",
        verbose_name="Slack Email Address",
    )
    ...
    block_execution = models.BooleanField(
        default=False,
        help_text=
...
class Product_Type(models.Model):
    ...
    def findings_count(self):
        return (Finding.objects.filter(
            ...
            duplicate=False,
            out_of_scope=False,
            test__engagement__product__prod_type=self,
        ).filter(
            Q(severity="Critical")
            | Q(severity="High")
            | Q(severity="Medium")
...
class Test_Type(models.Model):
    ...
    def get_breadcrumbs(self):
        bc = [{"title": self.__unicode__(), "url": None}]
        return bc
    ...
    def clean(self):
        if self.product_id is None and self.endpoint_id is None:
            raise ValidationError(
                "Metadata entries need either a product or an endpoint")
        if self.product_id is not None and self.endpoint_id is not None:
            raise ValidationError(
                "Metadata entries may not have both a product and an endpoint")
...
class Product(models.Model):
    ...
    """
    prod_manager = models.CharField(default=0,
                                    max_length=200,
                                    null=True,
                                    blank=True)  # unused
    tech_contact = models.CharField(default=0,
                                    max_length=200,
                                    null=True,
                                    blank=True)  # unused
    manager = models.CharField(default=0,
                               max_length=200,
                               null=True,
                               blank=True)  # unused
    ...
    platform = models.CharField(max_length=11,
                                ...
                                blank=True,
                                null=True)
    lifecycle = models.CharField(max_length=12,
    ...
    revenue = models.DecimalField(
        ...
        blank=True,
        null=True,
        help_text=_("Estimate the application's revenue."),
    )
    ...
    def findings_count(self):
        return Finding.objects.filter(
            ...
            duplicate=False,
            out_of_scope=False,
            test__engagement__product=self,
        ).count()
...
class ScanSettings(models.Model):
    ...
    def get_breadcrumbs(self):
        ...
        bc += [{
            ...
            "Scan Settings",
            "url":
            reverse("view_scan_settings", args=(
                self.product.id,
                self.id,
            )),
        }]
...
class Scan(models.Model):
    scan_settings = models.ForeignKey(ScanSettings,
                                      default=1,
                                      editable=False,
                                      on_delete=models.CASCADE)
    date = models.DateTimeField(editable=False,
...
class Tool_Configuration(models.Model):
    ...
    tool_type = models.ForeignKey(Tool_Type,
                                  related_name="tool_type",
                                  on_delete=models.CASCADE)
    authentication_type = models.CharField(
        max_length=15,
        choices=(("API", "API Key"), ("Password", "Username/Password"),
                 ("SSH", "SSH")),
        null=True,
        blank=True,
    )
...
class Engagement_Presets(models.Model):
    title = models.CharField(max_length=500,
                             default=None,
                             help_text="Brief description of preset.")
    test_type = models.ManyToManyField(Test_Type, default=None, blank=True)
    network_locations = models.ManyToManyField(Network_Locations,
                                               default=None,
                                               blank=True)
...
class Engagement(models.Model):
    ...
    eng_type = models.ForeignKey(Engagement_Type,
                                 null=True,
                                 blank=True,
                                 on_delete=models.CASCADE)
    ...
    tracker = models.URLField(
        max_length=200,
        help_text="Link to epic or ticket system with changes to version.",
        editable=True,
        blank=True,
        null=True,
    )
    ...
    progress = models.CharField(max_length=100,
                                ...
                                editable=False)
    tmodel_path = models.CharField(max_length=1000,
                                   default="none",
                                   editable=False,
                                   blank=True,
                                   null=True)
    risk_path = models.CharField(max_length=1000,
                                 default="none",
                                 editable=False,
                                 blank=True,
                                 null=True)
    ...
    engagement_type = models.CharField(
        ...
        default="Interactive",
        null=True,
        choices=(("Interactive", "Interactive"), ("CI/CD", "CI/CD")),
    )
    build_id = models.CharField(
        ...
        null=True,
        blank=True,
        help_text="Build ID of the product the engagement tested.",
    ...
    commit_hash = models.CharField(
        ...
        null=True,
        blank=True,
        help_text="Commit hash from repo",
        verbose_name="Commit Hash",
    )
    branch_tag = models.CharField(
        editable=True,
        max_length=150,
        null=True,
        blank=True,
        help_text="Tag or branch of the product the engagement tested.",
        verbose_name="Branch/Tag",
    )
    build_server = models.ForeignKey(
        Tool_Configuration,
        verbose_name="Build Server",
        help_text="Build server responsible for CI/CD test",
        null=True,
        blank=True,
        related_name="build_server",
        on_delete=models.CASCADE,
    )
    source_code_management_server = models.ForeignKey(
        Tool_Configuration,
        null=True,
        blank=True,
        verbose_name="SCM Server",
        help_text="Source code server for CI/CD test",
        related_name="source_code_management_server",
        on_delete=models.CASCADE,
    )
    source_code_management_uri = models.URLField(
        max_length=600,
        null=True,
        blank=True,
        editable=True,
    ...
    orchestration_engine = models.ForeignKey(
        Tool_Configuration,
        verbose_name="Orchestration Engine",
        help_text="Orchestration service responsible for CI/CD test",
        null=True,
        blank=True,
...
class Endpoint(models.Model):
    ...
    host = models.CharField(
        ...
        max_length=500,
        help_text=
        "The host name or IP address, you can also include the port number. For example"
    ...
    path = models.CharField(
        ...
        max_length=500,
        help_text=
        "The location of the resource, it should start with a '/'. For example"
        "/endpoint/420/edit",
    )
    ...
    fragment = models.CharField(
        ...
        max_length=500,
        help_text=
        "The fragment identifier which follows the hash mark. The hash mark should "
    ...
    product = models.ForeignKey(Product,
                                ...
                                blank=True,
                                on_delete=models.CASCADE)
    endpoint_params = models.ManyToManyField(Endpoint_Params,
                                             blank=True,
                                             editable=False)
    ...
    def __unicode__(self):
        ...
        if port:
            ...
            if (port != 443 and scheme == "https") or (port != 80
                                                       and scheme == "http"):
                netloc += ":%s" % port
    ...
    def active_findings(self):
        ...
        return (Finding.objects.filter(
            ...
            false_p=False,
            duplicate=False,
        ).distinct().order_by("numerical_severity"))
...
class NoteHistory(models.Model):
    ...
    time = models.DateTimeField(null=True,
                                editable=False,
                                default=get_current_datetime)
    current_editor = models.ForeignKey(User,
                                       editable=False,
                                       null=True,
                                       on_delete=models.CASCADE)
...
class Notes(models.Model):
    ...
    edit_time = models.DateTimeField(null=True,
                                     editable=False,
                                     default=get_current_datetime)
    history = models.ManyToManyField(NoteHistory, blank=True, editable=False)
...
class Test(models.Model):
    ...
    percent_complete = models.IntegerField(null=True,
                                           blank=True,
                                           editable=True)
    notes = models.ManyToManyField(Notes, blank=True, editable=False)
    environment = models.ForeignKey(Development_Environment,
                                    null=True,
                                    blank=False,
                                    on_delete=models.CASCADE)
...
class Finding(models.Model):
    ...
    duplicate_finding = models.ForeignKey(
        "self",
        editable=False,
        null=True,
    ...
    defect_review_requested_by = models.ForeignKey(
        Dojo_User,
        null=True,
        blank=True,
    ...
    mitigated_by = models.ForeignKey(
        User,
        null=True,
        editable=False,
    ...
    scanner_confidence = models.IntegerField(
        null=True,
        blank=True,
        default=None,
        editable=False,
        help_text=
        "Confidence level of vulnerability which is supplied by the scannner.",
    )
    ...
    def compute_hash_code(self):
        hash_string = (self.title + str(self.cwe) + str(self.line) +
                       str(self.file_path) + self.description)
    ...
    def long_desc(self):
        ...
        long_desc += ("*Product/Engagement:* " +
                      self.test.engagement.product.name + " / " +
                      self.test.engagement.name + "\n\n")
    ...
    def save(self,
        ...
        if dedupe_option and self.hash_code is not None:
            if system_settings.enable_deduplication:
                ...
                try:
                    if self.reporter.usercontactinfo.block_execution:
                        dedupe_signal.send(sender=self.__class__,
                                           new_finding=self)
                    else:
    ...
    def clean(self):
        ...
        bigfields = [
            "description", "mitigation", "references", "impact", "url"
        ]
...
class Stub_Finding(models.Model):
    ...
    def get_breadcrumbs(self):
        bc = self.test.get_breadcrumbs()
        bc += [{
            "title": "Potential Finding: " + self.__unicode__(),
...
class Finding_Template(models.Model):
    ...
    numerical_severity = models.CharField(max_length=4,
                                          ...
                                          blank=True,
                                          editable=False)
    template_match = models.BooleanField(
        default=False,
    ...
    template_match_title = models.BooleanField(
        default=False,
        verbose_name="Match Template by Title and CWE",
        help_text="Matches by title text (contains search) and CWE.",
    )
...
class Check_List(models.Model):
    ...
    data_input_sanitization_validation = models.CharField(max_length=50,
                                                          default="none")
    data_issues = models.ManyToManyField(Finding,
    ...
    other_issues = models.ManyToManyField(Finding,
                                          related_name="other_issues",
                                          blank=True)
    ...
    def get_breadcrumb(self):
        ...
        bc += [{
            ...
            "Check List",
            "url":
            reverse("complete_checklist", args=(self.engagement.id, )),
        }]
        return bc
...
class Risk_Acceptance(models.Model):
    path = models.FileField(
        ...
        null=False,
        blank=False,
        verbose_name="Risk Acceptance File",
    )
    ...
    accepted_by = models.CharField(
        ...
        help_text="The entity or person that accepts the risk.",
    )
    reporter = models.ForeignKey(User,
                                 editable=False,
                                 on_delete=models.CASCADE)
    ...
    def filename(self):
        return os.path.basename(
            self.path.name) if self.path is not None else ""
    ...
    def get_breadcrumbs(self):
        ...
        bc += [{
            ...
            reverse("view_risk",
                    args=(
                        self.engagement_set.first().product.id,
                        self.id,
                    )),
        }]
        return bc
...
class FindingImageAccessToken(models.Model):
    ...
    user = models.ForeignKey(User,
                             ...
                             blank=False,
                             on_delete=models.CASCADE)
    image = models.ForeignKey(FindingImage,
                              null=False,
                              blank=False,
                              on_delete=models.CASCADE)
...
class JIRA_Conf(models.Model):
    configuration_name = models.CharField(
        max_length=2000,
        help_text="Enter a name to give to this configuration",
        default="",
    )
    url = models.URLField(
        max_length=2000,
        ...
        help_text=
        "For configuring Jira, view: https://defectdojo.readthedocs.io/en/latest/features.html#jira-integration",
    )
    #    product = models.ForeignKey(Product)
    ...
    default_issue_type = models.CharField(
        max_length=9,
        choices=(
            ("Task", "Task"),
            ("Story", "Story"),
    ...
    open_status_key = models.IntegerField(
        help_text=
        "To obtain the 'open status key' visit https://<YOUR JIRA URL>/rest/api/latest/issue/<ANY VALID ISSUE KEY>/transitions?expand=transitions.fields"
    )
    close_status_key = models.IntegerField(
        help_text=
        "To obtain the 'open status key' visit https://<YOUR JIRA URL>/rest/api/latest/issue/<ANY VALID ISSUE KEY>/transitions?expand=transitions.fields"
    )
    info_mapping_severity = models.CharField(
        max_length=200,
        help_text="Maps to the 'Priority' field in Jira. For example: Info",
    )
    low_mapping_severity = models.CharField(
        max_length=200,
        help_text="Maps to the 'Priority' field in Jira. For example: Low",
    )
    medium_mapping_severity = models.CharField(
        max_length=200,
        help_text="Maps to the 'Priority' field in Jira. For example: Medium",
    )
    high_mapping_severity = models.CharField(
        max_length=200,
        help_text="Maps to the 'Priority' field in Jira. For example: High",
    )
    critical_mapping_severity = models.CharField(
        max_length=200,
        help_text="Maps to the 'Priority' field in Jira. For example: Critical",
    )
    ...
    accepted_mapping_resolution = models.CharField(
        null=True,
        blank=True,
        max_length=300,
        help_text=
    ...
    false_positive_mapping_resolution = models.CharField(
        null=True,
        blank=True,
        max_length=300,
        help_text=
    ...
    def accepted_resolutions(self):
        return [
            m.strip()
            for m in (self.accepted_mapping_resolution or "").split(",")
        ]
    ...
    def false_positive_resolutions(self):
        return [
            m.strip()
            for m in (self.false_positive_mapping_resolution or "").split(",")
        ]
...
class JIRA_Issue(models.Model):
    ...
    finding = models.OneToOneField(Finding,
                                   null=True,
                                   blank=True,
                                   on_delete=models.CASCADE)
    engagement = models.OneToOneField(Engagement,
                                      null=True,
                                      blank=True,
                                      on_delete=models.CASCADE)
    ...
    def __unicode__(self):
        text = ""
        if self.finding:
            text = (self.finding.test.engagement.product.name +
                    " | Finding: " + self.finding.title + ", ID: " +
                    str(self.finding.id))
        elif self.engagement:
            text = (self.engagement.product.name + " | Engagement: " +
                    self.engagement.name + ", ID: " + str(self.engagement.id))
...
class Notifications(models.Model):
    product_added = MultiSelectField(choices=NOTIFICATION_CHOICES,
                                     default="alert",
                                     blank=True)
    ...
    upcoming_engagement = MultiSelectField(choices=NOTIFICATION_CHOICES,
                                           default="alert",
                                           blank=True)
    stale_engagement = MultiSelectField(choices=NOTIFICATION_CHOICES,
                                        default="alert",
                                        blank=True)
    auto_close_engagement = MultiSelectField(choices=NOTIFICATION_CHOICES,
                                             default="alert",
                                             blank=True)
    user_mentioned = MultiSelectField(choices=NOTIFICATION_CHOICES,
                                      default="alert",
                                      blank=True)
    code_review = MultiSelectField(choices=NOTIFICATION_CHOICES,
                                   default="alert",
                                   blank=True)
    review_requested = MultiSelectField(choices=NOTIFICATION_CHOICES,
                                        default="alert",
                                        blank=True)
    other = MultiSelectField(choices=NOTIFICATION_CHOICES,
                             default="alert",
                             blank=True)
    user = models.ForeignKey(User,
                             default=None,
...
class Tool_Product_Settings(models.Model):
    ...
    product = models.ForeignKey(Product,
                                ...
                                editable=False,
                                on_delete=models.CASCADE)
    tool_configuration = models.ForeignKey(
        Tool_Configuration,
        null=False,
...
class Tool_Product_History(models.Model):
    product = models.ForeignKey(Tool_Product_Settings,
                                editable=False,
                                on_delete=models.CASCADE)
    ...
    configuration_details = models.CharField(max_length=2000,
                                             null=True,
                                             blank=True)
...
class Cred_User(models.Model):
    ...
    authentication = models.CharField(
        max_length=15,
        choices=(("Form", "Form Authentication"), ("SSO", "SSO Redirect")),
        default="Form",
    )
    http_authentication = models.CharField(
        max_length=15,
        choices=(("Basic", "Basic"), ("NTLM", "NTLM")),
        null=True,
        blank=True,
    )
    description = models.CharField(max_length=2000, null=True, blank=True)
...
class Languages(models.Model):
    ...
    files = models.IntegerField(blank=True,
                                null=True,
                                verbose_name="Number of files")
    blank = models.IntegerField(blank=True,
                                null=True,
                                verbose_name="Number of blank lines")
    comment = models.IntegerField(blank=True,
                                  null=True,
                                  verbose_name="Number of comment lines")
    code = models.IntegerField(blank=True,
                               null=True,
                               verbose_name="Number of code lines")
    created = models.DateTimeField(null=False, editable=False, default=now)
...
class Objects(models.Model):
    ...
    path = models.CharField(max_length=600,
                            verbose_name="Full file path",
                            null=True,
                            blank=True)
    folder = models.CharField(max_length=400,
                              verbose_name="Folder",
                              null=True,
                              blank=True)
    artifact = models.CharField(max_length=400,
...
class Objects_Engagement(models.Model):
    ...
    def __unicode__(self):
        ...
        return data + " | " + self.engagement.name + " | " + str(
            self.engagement.id)
...
class Testing_Guide(models.Model):
    ...
    identifier = models.CharField(max_length=20,
                                  blank=True,
                                  null=True,
                                  help_text="Test Unique Identifier")
    ...
    objective = models.CharField(max_length=800,
                                 help_text="Objective of the test")
    how_to_test = models.TextField(default=None,
                                   help_text="How to test the objective")
    results_expected = models.CharField(
        max_length=800, help_text="What the results look like for a test")
    created = models.DateTimeField(null=False, editable=False, default=now)
    updated = models.DateTimeField(editable=False, default=now)
...
class Benchmark_Type(models.Model):
    ...
    benchmark_source = models.CharField(max_length=20,
                                        blank=False,
                                        null=True,
                                        choices=source,
                                        default="OWASP ASVS")
...
class Benchmark_Category(models.Model):
    type = models.ForeignKey(Benchmark_Type,
...
class Benchmark_Product(models.Model):
    ...
    enabled = models.BooleanField(
        default=True, help_text="Applicable for this specific product.")
    notes = models.ManyToManyField(Notes, blank=True, editable=False)
    created = models.DateTimeField(null=False, editable=False, default=now)
    updated = models.DateTimeField(editable=False, default=now)

    def __unicode__(self):
        return (self.product.name + ": " + self.control.objective_number +
                ": " + self.control.category.name)
...
class Benchmark_Product_Summary(models.Model):
    ...
    current_level = models.CharField(max_length=15,
                                     ...
                                     choices=asvs_level,
                                     default="None")
    asvs_level_1_benchmark = models.IntegerField(
        null=False,
        default=0,
        help_text="Total number of active benchmarks for this application.",
    )
    asvs_level_1_score = models.IntegerField(null=False,
                                             default=0,
                                             help_text="ASVS Level 1 Score")
    asvs_level_2_benchmark = models.IntegerField(
        null=False,
        default=0,
        help_text="Total number of active benchmarks for this application.",
    )
    asvs_level_2_score = models.IntegerField(null=False,
                                             default=0,
                                             help_text="ASVS Level 2 Score")
    asvs_level_3_benchmark = models.IntegerField(
        null=False,
        default=0,
        help_text="Total number of active benchmarks for this application.",
    )
    asvs_level_3_score = models.IntegerField(null=False,
                                             default=0,
                                             help_text="ASVS Level 3 Score")
    publish = models.BooleanField(default=False,
                                  help_text="Publish score to Product.")
    created = models.DateTimeField(null=False, editable=False, default=now)
    updated = models.DateTimeField(editable=False, default=now)
...
operator_options = (("Matches", "Matches"), ("Contains", "Contains"))
application_options = (("Append", "Append"), ("Replace", "Replace"))
blank_options = (("", ""), )
...
class FieldRule(models.Model):
    ...
    update_options = (("Append", "Append"), ("Replace", "Replace"))
