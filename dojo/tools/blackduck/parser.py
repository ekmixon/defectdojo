...
class BlackduckHubCSVParser(object):
    ...
    def __init__(self, filename, test):
        ...
        for i, row in df.iterrows():
            ...
            dupe_key = hashlib.md5((
                title + "|" +
                df.ix[i, "Vulnerability source"]).encode("utf-8")).hexdigest()
    ...
    def format_mitigation(self, df, i):
        mitigation = "Remediation status: {}\n".format(
            df.ix[i, "Remediation status"])
        mitigation += "Remediation target date: {}\n".format(
            df.ix[i, "Remediation target date"])
        mitigation += "Remdediation actual date: {}\n".format(
            df.ix[i, "Remediation actual date"])
        mitigation += "Remdediation comment: {}\n".format(
            df.ix[i, "Remediation comment"])
