    ...
    def get_items(self, tree, test):
        ...
        if "vulnerabilities" in tree:
            ...
            for node in vulnerabilityTree:
                ...
                unique_key = node["title"] + str(node["packageName"] +
                                                 str(node["version"]) +
                                                 str(node["from"]))
...
def get_item(vulnerability, test):
    ...
    if "identifiers" in vulnerability:
        if "CVE" in vulnerability["identifiers"]:
            if isinstance(vulnerability["identifiers"]["CVE"], list):
                ...
                cve = " ".join(
                    vulnerability["identifiers"]["CVE"]).split(" ")[0]
        ...
        if "CWE" in vulnerability["identifiers"]:
            if isinstance(vulnerability["identifiers"]["CWE"], list):
                ...
                cwe = (" ".join(vulnerability["identifiers"]["CWE"]).split(" ")
                       [0].split("-")[1])
                if len(vulnerability["identifiers"]["CVE"]) > 1:
                    cwe_references = ", ".join(
                ...
                # in case the structure is not a list?
                cwe = "".join(
                    vulnerability["identifiers"]["CWE"]).split("-")[1]
    else:
    ...
    finding = Finding(
        ...
        vulnerability["packageName"] + "</li><li> Current Version: " +
        str(vulnerability["version"]) + "</li><li>Vulnerable Version(s): " +
        vulnerable_versions + "</li><li>Vulnerable Path: " +
        " > ".join(vulnerability["from"]) + "</li></p>" +
