            ...
            for node in vulnerabilityTree:
                ...
                if "cves" in more_details:
                    if "cve" in more_details.get("cves")[0]:
                        title_cve = (node.get("component_versions").get(
                            "more_details").get("cves")[0].get("cve"))

                unique_key = (node.get("id") + node.get("summary") +
                              node.get("provider") +
                              node.get("source_comp_id") + title_cve)
...
def get_item(vulnerability, test):
    ...
    if "cves" in vulnerability["component_versions"]["more_details"]:
        if "cve" in vulnerability["component_versions"]["more_details"][
                "cves"][0]:
            cve = vulnerability["component_versions"]["more_details"]["cves"][
                0]["cve"]

        if "cwe" in vulnerability["component_versions"]["more_details"][
                "cves"][0]:
            # take only the first one for now, limitation of DD model.
            cwe = vulnerability["component_versions"]["more_details"]["cves"][
                0]["cwe"][0].split("-")[1]
        ...
        if "cvss_v3" in vulnerability["component_versions"]["more_details"][
                "cves"][0]:
            cvss_v3 = vulnerability["component_versions"]["more_details"][
                "cves"][0]["cvss_v3"]

        if "cvss_v2" in vulnerability["component_versions"]["more_details"][
                "cves"][0]:
            cvss_v2 = vulnerability["component_versions"]["more_details"][
                "cves"][0]["cvss_v2"]

    if "fixed_versions" in vulnerability["component_versions"]:
        mitigation = "Versions containing a fix:\n"
        mitigation = mitigation + "</br>".join(
    ...
    finding = Finding(
        ...
        references=vulnerability.get("component_versions").get(
            "more_details").get("provider"),
        impact=severity,
    )
