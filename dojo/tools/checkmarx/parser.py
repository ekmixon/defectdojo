    ...
    def __init__(self, filename, test):
        ...
        for query in root.findall("Query"):
            ...
            for result in query.findall("Result"):
                ...
                dupe_key = "{}{}{}{}".format(
                    categories, cwe, name,
                    result.get("FileName").encode("utf-8"))

                if dupe_key in dupes:
                    find = dupes[dupe_key]
                    title, description, pathnode = self.get_finding_detail(
                        query, result)
    ...
    def get_finding_detail(self, query, result):
        ...
        for path in result.findall("Path"):
            ...
            for pathnode in path.findall("PathNode"):
                result_dupes_key = (pathnode.find("Line").text + "|" +
                                    pathnode.find("Column").text)
        ...
        if title and pathnode.find("FileName").text:
            title = "{} ({})".format(
                title, ntpath.basename(pathnode.find("FileName").text))
