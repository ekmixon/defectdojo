    ...
    def get_items(self, tree, test):
        ...
        if "results" in tree:
            ...
            for node in vulnerabilityTree:
                ...
                unique_key = node["id"] + str(node["packageName"] +
...
def get_item(vulnerability, test):
    ...
    vector = (vulnerability["vector"]
              if "vector" in vulnerability else "CVSS vector not provided. ")
    status = (vulnerability["status"] if "status" in vulnerability else
              "There seems to be no fix yet. Please check description field.")
    cvss = vulnerability[
        "cvss"] if "cvss" in vulnerability else "No CVSS score yet."
    riskFactors = (vulnerability["riskFactors"]
                   if "riskFactors" in vulnerability else "No risk factors.")

    # create the finding object
    finding = Finding(
        title=vulnerability["id"] + ": " + vulnerability["packageName"] +
        " - " + vulnerability["packageVersion"],
        cve=vulnerability["id"],
        test=test,
        severity=severity,
        description=vulnerability["description"] + "<p> Vulnerable Package: " +
        vulnerability["packageName"] + "</p><p> Current Version: " +
        str(vulnerability["packageVersion"]) + "</p>",
