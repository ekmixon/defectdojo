...
def get_item(item_node, test):
    ...
    if request is not None and respz is not None:
        unsaved_req_resp.append({
            "req": b64encode(req.encode("utf-8")),
            "resp": b64encode(resp.encode("utf-8")),
        })
    ...
    background = item_node[
        "description"] if "description" in item_node else None
    if background:
        background = html2text.html2text(background)

    remediation = (item_node["remedy_guidance"]
                   if "remedy_guidance" in item_node else "n/a")
    if remediation:
        remediation = html2text.html2text(remediation)

    references = (list(item_node["references"].values())
                  if "references" in item_node else None)
    references = "<br/><br/>".join(reference for reference in references)

    if references:
        references = html2text.html2text(references)

    severity = item_node["severity"].capitalize(
    ) if "severity" in item_node else "Info"
    ...
    finding = Finding(
        title=item_node["name"] + " (" + str(digest) + ")",
        url=url,
        test=test,
        severity=severity,
