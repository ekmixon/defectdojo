...
class OpenscapXMLParser(object):
    def __init__(self, file, test):
        ...
        for rule, rule_result in zip(
                root.findall("./{0}Rule".format(namespace)),
                test_result.findall("./{0}rule-result".format(namespace)),
        ):
            cves = []
            ...
            if "fail" in result:
                ...
                if len(cves) > 1:
                    ...
                    for cve in cves:
                        cve_desc += (
                            "[{0}](https://cve.mitre.org/cgi-bin/cvename.cgi?name={0})"
                            .format(cve) + ", ")
                ...
                check_content = rule_result.find(
                    "./{0}check/{0}check-content-ref".format(namespace)).attrib
                # get references.
