...
class VCGXmlParser(object):
    ...
    def get_field_from_xml(issue, field):
        if issue.find(field) is not None and issue.find(
                field).text is not None:
    ...
    def parse_issue(self, issue, test):
        ...
        data.title = ("" if self.get_field_from_xml(issue, "Title") is None
