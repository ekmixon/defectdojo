    ...
    def get_finding_from_vulnerability(self, vulnerability, filename, test):
        ...
        if cvssv3_node is not None:
            severity = (self.get_field_value(
                cvssv3_node, "baseSeverity").lower().capitalize())
        elif cvssv2_node is not None:
            severity = (self.get_field_value(cvssv2_node,
                                             "severity").lower().capitalize())
        else:
            severity = (self.get_field_value(vulnerability,
                                             "severity").lower().capitalize())
                    ...
                    for vulnerability in vulnerabilities.findall(
                        ...
                        if finding is not None:
                            key_str = "{}|{}|{}".format(
                                ...
                                finding.description)
                            key = hashlib.md5(
                                key_str.encode("utf-8")).hexdigest()
