...
class CoverPageForm(forms.Form):
    ...
    sub_heading = forms.CharField(max_length=200,
                                  required=False,
...
class TableOfContentsForm(forms.Form):
    ...
    depth = forms.IntegerField(min_value=1,
                               required=False,
                               max_value=6,
                               initial=4)
...
class PageBreak(Widget):
    ...
    def get_option_form(self):
        return mark_safe(
            ...
            + self.get_html() +
            "</h5><span class='fa fa-arrows pull-right icon'></span></div></div>"
...
class ReportOptions(Widget):
    def __init__(self, *args, **kwargs):
        ...
        self.extra_help = (
            "Choose additional report options.  These will apply to the overall report."
        )
...
class TableOfContents(Widget):
    def __init__(self, *args, **kwargs):
        ...
        self.help_text = (
            "The table of contents includes a page break after its content.")

    def get_html(self):
        return mark_safe("")

    def get_asciidoc(self):
        return render_to_string("dojo/custom_asciidoc_toc.html", {
            "title": self.title,
            "depth": self.depth
        })
...
class WYSIWYGContent(Widget):
    ...
    def get_html(self):
        html = render_to_string(
            "dojo/custom_pdf_report_wysiwyg_content.html",
            {
                "title": self.title,
...
class FindingList(Widget):
    def __init__(self, *args, **kwargs):
        ...
        self.multiple = "true"
        self.extra_help = (
            "You can use this form to filter findings and select only the ones to be included in the "
            "report.")
        title_words = [
            word for finding in self.findings.qs
        ...
        if self.request is not None:
            self.paged_findings = get_page_items(self.request,
                                                 self.findings.qs, 25)
        else:
    ...
    def get_html(self):
        html = render_to_string(
            "dojo/custom_pdf_report_finding_list.html",
            {
                "title": self.title,
                "findings": self.findings.qs,
                "include_finding_notes": self.finding_notes,
...
class EndpointList(Widget):
    def __init__(self, *args, **kwargs):
        ...
        self.multiple = "false"
        if self.request is not None:
            self.paged_endpoints = get_page_items(self.request,
                                                  self.endpoints.qs, 25)
        else:
            self.paged_endpoints = self.endpoints
        self.multiple = "true"
        self.extra_help = (
            "You can use this form to filter endpoints and select only the ones to be included in the "
            "report.")

    def get_html(self):
        html = render_to_string(
            "dojo/custom_pdf_report_endpoint_list.html",
            {
                "title": self.title,
...
def report_widget_factory(
    ...
    for idx, widget in enumerate(widgets):
        ...
        if list(widget.keys())[0] == "finding-list":
            ...
            findings = ReportAuthedFindingFilter(d,
                                                 queryset=findings,
                                                 user=user)
            user_id = user.id if user is not None else None
            selected_widgets[list(widget.keys())[0] + "-" +
