...
def async_dupe_delete(*args, **kwargs):
    ...
    if system_settings.delete_dupulicates:
        ...
        findings = (Finding.objects.filter(
            duplicate_list__duplicate=True).annotate(
                num_dupes=Count("duplicate_list")).filter(
                    num_dupes__gt=dupe_max))
        for finding in findings:
            duplicate_list = finding.duplicate_list.filter(
