...
def metrics(request, mtype):
    ...
    if len(prod_type) > 0:
        ...
        accepted_findings = Finding.objects.filter(
            risk_acceptance__created__range=[start_date, end_date],
            test__engagement__product__prod_type__in=prod_type,
        ).prefetch_related("test__engagement__product")
        accepted_findings_counts = (Finding.objects.filter(
            risk_acceptance__created__range=[start_date, end_date],
            test__engagement__product__prod_type__in=prod_type,
            ...
            low=Sum(
                Case(When(severity="Low", then=Value(1)),
                     output_field=IntegerField())),
            info=Sum(
                Case(
    ...
    weekly_counts = get_period_counts(
        active_findings.qs,
        findings.qs,
        findings_closed,
        accepted_findings,
    ...
    top_ten = (Product.objects.filter(
        ...
        critical=Sum(
            Case(
                When(engagement__test__finding__severity="Critical",
                     then=Value(1)),
                output_field=IntegerField(),
            )),
        high=Sum(
            Case(
                When(engagement__test__finding__severity="High",
                     then=Value(1)),
                output_field=IntegerField(),
            )),
        medium=Sum(
            Case(
                When(engagement__test__finding__severity="Medium",
                     then=Value(1)),
                output_field=IntegerField(),
            )),
        low=Sum(
            Case(
                When(engagement__test__finding__severity="Low", then=Value(1)),
                output_field=IntegerField(),
            )),
        total=Sum(
            Case(
...
def simple_metrics(request):
    ...
    for pt in Product_Type.objects.order_by("name"):
        ...
        for f in total.all():
            ...
            if (f.mitigated and f.mitigated.year == now.year
...
def product_type_counts(request):
    ...
    mid_month = first_of_month.replace(day=15,
                                       ...
                                       second=59,
                                       microsecond=999999)
    end_of_month = mid_month.replace(
    ...
    if (request.method == "GET" and "month" in request.GET
            and "year" in request.GET and "product_type" in request.GET):
        ...
        if form.is_valid():
            ...
            end_of_month = month_requested.replace(
                day=monthrange(month_requested.year, month_requested.month)[1],
                hour=23,
                minute=59,
                second=59,
                microsecond=999999,
            ...
            for x in range(12, 0, -1):
                opened_in_period_list.append(
                    opened_in_period(
                        start_date + relativedelta(months=-x),
            ...
            top_ten = (Product.objects.filter(
                ...
                high=Sum(
                    Case(
                        When(
                            engagement__test__finding__severity="High",
                            then=Value(1),
                        ),
                        output_field=IntegerField(),
                    )),
                medium=Sum(
                    Case(
                        When(
                            engagement__test__finding__severity="Medium",
                            then=Value(1),
                        ),
                        output_field=IntegerField(),
                    )),
                low=Sum(
                    Case(
                        When(engagement__test__finding__severity="Low",
                             then=Value(1)),
                        output_field=IntegerField(),
                    )),
                total=Sum(
...
def engineer_metrics(request):
    ...
    users = EngineerFilter(request.GET, queryset=users)
...
def view_engineer(request, eid):
    ...
    if not (request.user.is_superuser or request.user.username == "root"
    ...
    for week in week_a_stuff:
        ...
        week_end = datetime.strptime(wk_range[1].strip() + " " + str(now.year),
                                     "%b %d %Y")
    ...
    for t in top:
        ...
        t_count = 0
        h_count = 0
        engs = Engagement.objects.filter(product=Product.objects.get(
            id=product))
        for eng in engs:
            tests = Test.objects.filter(engagement=eng)
            for test in tests:
                z_count += findings.filter(test=test,
                                           mitigated__isnull=True,
                                           severity="Critical").count()
                o_count += findings.filter(test=test,
                                           mitigated__isnull=True,
                                           severity="High").count()
                t_count += findings.filter(test=test,
                                           mitigated__isnull=True,
                                           severity="Medium").count()
                h_count += findings.filter(test=test,
                                           mitigated__isnull=True,
                                           severity="Low").count()
        prod = Product.objects.get(id=product)
        all_findings_link = "<a href='%s'>%s</a>" % (
            reverse("view_product_findings", args=(prod.id, )),
            escape(prod.name),
        )
        update.append([
            all_findings_link,
    ...
    findz = findings.filter(mitigated__isnull=True,
                            active=True,
